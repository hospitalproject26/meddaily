...
function Dashboard() {
  const dueRefills = regularCustomers
    .filter((c) => c.regularMedicine)
    .map((c) => ({ customer: c, refill: getRefillStatus(c.regularMedicine!) }))
    .filter((r) => r.refill.dueSoon);
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 p-4 sm:p-6 lg:p-8 space-y-8 relative overflow-hidden">
      ...
      {/* Regular Customer Refill Reminders */}
      {dueRefills.length > 0 && (
        <div className="glass-panel rounded-3xl p-6 space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h2 className="text-base font-bold text-slate-900">Refill Reminders</h2>
                <p className="text-[11px] text-slate-500 font-medium">
                  Regular customers finishing their medicine within 5 days
                </p>
              </div>
            </div>
            <span className="px-2.5 py-1 bg-indigo-50 text-indigo-700 border border-indigo-200/60 rounded-full text-xs font-bold">
              {dueRefills.length} Due Soon
            </span>
          </div>

          <div className="space-y-2.5">
            {dueRefills.map(({ customer, refill }) => (
              <div
                key={customer.id}
                className="p-3.5 bg-indigo-50/50 border border-indigo-200/50 rounded-2xl flex items-center justify-between"
              >
                <div>
                  <p className="text-xs font-extrabold text-slate-900">{customer.name}</p>
                  <p className="text-[11px] font-medium text-slate-500">
                    {customer.regularMedicine?.name} · {customer.phone}
                  </p>
                </div>
                <span className="px-3 py-1 bg-indigo-100 text-indigo-900 rounded-xl text-xs font-black">
                  {refill.daysRemaining <= 0 ? "Finishes today" : `${refill.daysRemaining}d left`}
                </span>
              </div>
            ))}
          </div>
